...
class features_set:
    def __init__(self, feature_path=None, feature_column=[], feature_column_to_drop=[],
                 outcome_column='', patient_column='',
                 patient_to_drop=[]):
    ...
    def __read_file(self):
        ...
        if self._outcome_column:
            self._class_label = np.unique(np.array(list(feature_df[self._outcome_column])))
        else:
            print('No outcome column specified')
        ...
        if self._patient_column:
            ...
            self._feature_dataframe = feature_df
    ...
    def handle_nan(self, axis=1, how='any', mode='delete'):
        if mode == 'delete':
            ...
            self._patient_name = list(self._feature_dataframe.index)
            ...
            if len(self._class_label) == 2:
                ...
                for feature in num_features:
                    ...
                    fig.append_trace(go.Histogram(
                        x=self._feature_dataframe.loc[self._feature_dataframe[self._outcome_column] ==
    ...
    def __get_MW_p(self, ftrs, p_threshold=0.05):
        p_MW = []
        df_0 = self._feature_dataframe.loc[self._feature_dataframe[self._outcome_column] ==
                                           self._class_label[0]]
        df_1 = self._feature_dataframe.loc[self._feature_dataframe[self._outcome_column] ==
                                           self._class_label[1]]
        for feature in ftrs:
    ...
    def __get_univar_fprs_tprs(self, ftr):
        ...
        mean_0 = self._feature_dataframe.loc[self._feature_dataframe[self._outcome_column] ==
                                             self._class_label[0]][ftr].mean()
        mean_1 = self._feature_dataframe.loc[self._feature_dataframe[self._outcome_column] ==
                                             self._class_label[1]][ftr].mean()
        feature_min = np.min(np.array(self._feature_dataframe[ftr]))
        feature_max = np.max(np.array(self._feature_dataframe[ftr]))
        step = (feature_max - feature_min) / 100
        x = np.array(self._feature_dataframe[ftr])
        n_0 = len(self._feature_dataframe.loc[self._feature_dataframe[self._outcome_column] ==
                                              self._class_label[0]])
        n_1 = len(self._feature_dataframe.loc[self._feature_dataframe[self._outcome_column] ==
                                              self._class_label[1]])
        th = []
        tprs = []
        fprs = []
        ...
        for i in range(0, 102):
            ...
            tp = np.sum(y_pred[np.where(self._feature_dataframe[self._outcome_column] ==
                                        self._class_label[1])])
            fp = np.sum(y_pred[np.where(self._feature_dataframe[self._outcome_column] ==
                                        self._class_label[0])])
            tpr = tp / n_1
            fpr = fp / n_0
            tprs.append(tpr)
    ...
    def calculate_basic_stats(self, volume_feature=''):
        ...
        stats_dataframe.to_csv(os.path.splitext(self._feature_path)[0] + '_basic_stats.csv')
